...
#SBATCH -J pi_multik_sweep_independent_conv_flatten_aux
#SBATCH -p sae
...
#SBATCH --gres=gpu:1
#SBATCH --array=0-4
#SBATCH --output=logs/pi_multik_sweep_independent_conv_flatten_aux_%A_%a.out
#SBATCH --error=logs/pi_multik_sweep_independent_conv_flatten_aux_%A_%a.err

# pi_multik encoder/fusion sweep, WITH auxiliary variables -- combo 6/6:
# encoder_mode=independent, fusion_mode=conv, fusion_pool=flatten,
# include_entropy=true. See pi_multik_sweep_shared_concat_aux.sh's header
# for what include_entropy adds. Same architecture as
# encsweep_independent_conv_flatten (== the old pi_multik_towers preset;
# the only combo that converged cleanly on all 5 seeds in the original
# sweep -- see the loss-comparison report). nested_thomas, dtm k=5,10,15
...
python -u scripts/train.py configs/runs/nested_thomas/pi_multik.yaml \
  --seed "$SEED" \
  --run-tag encsweep_independent_conv_flatten_aux \
