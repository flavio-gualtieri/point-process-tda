...
#SBATCH -J pi_multik_calib_resolution_nested_thomas
#SBATCH -p sae
...
#SBATCH --gres=gpu:1
#SBATCH --array=0-8
#SBATCH --output=logs/pi_multik_calib_resolution_nested_thomas_%A_%a.out
#SBATCH --error=logs/pi_multik_calib_resolution_nested_thomas_%A_%a.err

# PI raster resolution sweep, nested_thomas, FUSED DTM k=5,10,15 (one
...
# models), 3 values of raster resolution in pixels per side
# (method.params.resolution). 3 seeds (9371, 9372, 9373).
...
# results/nested_thomas/dtm_k5+10+15/pi_multik/_runs/calib_resolution<value>/seed_<seed>/
...
#   sbatch slurm/pi_multik_calib_resolution_nested_thomas.sh
