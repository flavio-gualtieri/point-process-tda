...
#SBATCH -J betti_multik_fairq_nested_thomas
#SBATCH -p sae
...
#SBATCH --gres=gpu:1
#SBATCH --array=0-5
#SBATCH --output=logs/betti_multik_fairq_nested_thomas_%A_%a.out
#SBATCH --error=logs/betti_multik_fairq_nested_thomas_%A_%a.err

# nested_thomas sibling of betti_multik_fairq_thomas.sh -- see that file's
# header for the full explanation. BC arm (betti_multik, H0+H1,
# include_euler=false), FUSED DTM k=5,10,15, 2 values of q x 3 seeds
# (9371, 9372, 9373), range_pad fixed at 1.05 on every task. Base config:
