...
#SBATCH -J pi_multik_sweep_independent_concat
#SBATCH -p sae
...
#SBATCH --gres=gpu:1
#SBATCH --array=0-4
#SBATCH --output=logs/pi_multik_sweep_independent_concat_%A_%a.out
#SBATCH --error=logs/pi_multik_sweep_independent_concat_%A_%a.err

# pi_multik encoder/fusion sweep -- combo 4/6: encoder_mode=independent,
# fusion_mode=concat (NEW combination: n_k separate, non-weight-shared
# CoordConvPIEncoders -- one per k -- flat-concatenated, no fusion conv --
# see EncoderBank's docstring in src/cloudforger/encoders/encoder_bank.py).
