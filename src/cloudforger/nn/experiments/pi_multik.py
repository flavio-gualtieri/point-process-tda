...
image computed at EACH DTM k in cfg["k_values"] into one
(2*len(k_values))-channel image, fed through a single PIEncoder. Unlike
fusion.py's L(r)-r+TDA fusion (several different MODALITIES, late-fused via
concatenation), this fuses the SAME modality (persistence images) at several
density SCALES (k) by stacking them as channels into ONE encoder, so a
single conv kernel can see all k's jointly rather than only meeting them at
a final concat layer. Absorbed from dtm_experiment/pi_multik_model.py.

Channel order (fixed, documented here since nothing in the tensor itself
labels it): for k in cfg["k_values"], in order: [pi0(k), pi1(k)]. E.g. for
k_values = [5, 10, 15]:
