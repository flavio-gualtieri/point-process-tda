# src/cloudforger/nn/experiments/combined_all.py
...
@register("combined_all")
class CombinedAllExperiment(Experiment):
    ...
    def _sibling_betti_path(dataset_path: Path) -> Path:
        ...
        if name == dataset_path.name:
            raise ValueError(
                f"combined_all expects dataset_path's filename to contain "
    ...
    def build_dataset(self, payload, labels):
        ...
        if not betti_path.exists():
            raise FileNotFoundError(
                f"combined_all needs the sibling betti file {betti_path} "
