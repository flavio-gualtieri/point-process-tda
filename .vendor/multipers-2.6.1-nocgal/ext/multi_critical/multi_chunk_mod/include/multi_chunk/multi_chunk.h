...
namespace multi_chunk {
  ...
  template<typename GrMat> void compress(std::vector<GrMat>& input_matrices,
					   bool bring_matrices_to_lex_order=false) {


    
      
    if(bring_matrices_to_lex_order) {
	if(multi_chunk::verbose) std::cout << "Sorting matrices" << std::endl;
	for(int i=0;i<input_matrices.size()-1;i++) {
	    GrMat& mat = input_matrices[i];
	    if(! mpp_utils::is_lex_sorted(mat) ) {
		mpp_utils::to_lex_order(mat,true,false);
	    }
	}
	if(! mpp_utils::is_lex_sorted(input_matrices.back())) {
	    mpp_utils::to_lex_order(input_matrices.back(),false,false);
	}
	if(multi_chunk::verbose) std::cout << "done sorting matrices" << std::endl;
    }
