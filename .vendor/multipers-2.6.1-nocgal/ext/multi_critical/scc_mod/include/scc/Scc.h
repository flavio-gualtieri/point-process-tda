...
namespace scc {
  ...
  bool get_next_line(std::istream& istr, 
    ...
    if(throw_error_on_fail) {
      throw ParseError("Unexpected end of file (1)");
    }
    return false;
  }
  ...
  class Scc {
	...
	bool is_there_next_line = get_next_line(istr,next,false);

	if(is_there_next_line) {
	...
	while(this->_levels[n].read_next_column(istr,true,false)) {
	  ...
	  if(count_last_level >= this->_levels[n].number_of_generators()) {
	    throw ParseError("Unexpected line "+this->_levels[n].next);
	  }
	  count_last_level++;
	}
	if(count_last_level>0 && count_last_level < this->_levels[n].number_of_generators()) {
	  throw ParseError("Unexpected end of file (2)");
	}
    ...
    void get_range_of_grades(Parser& scc, std::vector<std::pair<double,double>>& ranges) {

	int d = scc.number_of_parameters();
	ranges.resize(d);
	for(int i=0;i<d;i++) {
	    ranges[i].first=std::numeric_limits<double>::max();
	    ranges[i].second=std::numeric_limits<double>::min();
	}
	int L = scc.number_of_levels();
	for(int i=1;i<=L;i++) {
	    scc.reset(i);
	    while(scc.has_next_column(i)) {
		std::vector<double> grades;
		std::vector<std::pair<long,double>> bd;
		scc.next_column(i,std::back_inserter(grades),std::back_inserter(bd));
		assert(grades.size()%d==0);
		for(int j=0;j<grades.size();j+=d) {
		    for(int k=0;k<d;k++) {
			if(grades[j+k]<ranges[k].first) {
			    ranges[k].first=grades[j+k];
			}
			if(grades[j+k]>ranges[k].second) {
			    ranges[k].second=grades[j+k];
			}
		    }
		}
	    }
