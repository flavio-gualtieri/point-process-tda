...
per arm. $^\dagger$ one of five Thomas seeds diverged to the design-distribution-mean
...
Topology removed to $\log N$ alone collapses to the same
$\mathcal{L}\approx0.9$--$1.0$ floor as the label-shuffle control of
Section~\ref{ssec:exp-summaries} on both datasets, so the point count carries
almost none of the recoverable signal on its own. Removing $\log N$ from the
full model, conversely, costs little on nested-Thomas ($0.196\to0.200$) and is
...
both fine and coarse summaries. Unless a sweep varies it explicitly, every
experiment in this paper fuses $k \in \{5, 10, 15\}$; the encoder architecture
...
\expt{The [PI | H0+H1 | DTM\{5,10,15\} | 3 | q $\in$ \{0.90,0.95,0.99,1.00\}]
arm ($q\to1$ recovering naive min/max) is now run, alongside pad, kernel
width, and resolution -- see Table~\ref{tab:calibration-sweep} in
Section~\ref{sec:experiments}. Still missing: the train-only vs
full-population leakage arm.}
...
\begin{figure}[htbp]
    ...
    \caption{The same $H_0$ diagram (DTM filtration, $k=5$ -- a plain Rips
    filtration gives every point birth $0$, so the birth axis this figure
    calibrates would be degenerate) -- the most feature-rich diagram among
    the non-outlier bulk (own max persistence at or below the 60th
    percentile), drawn from $2000$ clouds sampled from the production
    Thomas design distribution (Section~\ref{sec:simulator}) -- rendered
    under the naive pooled min/max bound (left) and the per-diagram
    coverage-quantile bound of $q=0.95$, $\alpha=1.05$ (right). Top row: the
    diagram's own points against the calibrated birth-persistence box, with
    the dashed square marking the region zoomed into below; the naive box is
    stretched by a single high-offspring outlier diagram elsewhere in the
    population, not shown. Bottom row: the same physical region of
    birth-persistence space, rendered at each calibration's native pixel
    density -- a $7\times7$ block of mostly-fused pixels under the naive
    bound against a $16\times16$ grid that keeps the secondary peak resolved
    under the quantile bound, since the same $64\times64$ grid is stretched
    over a wider range.}
...
explicit coordinate channels. \todo{Still worth the calibrated-vs-naive
cross suggested above -- the claim that CoordConv helps more under calibrated
axes than uncalibrated ones has not been tested, only CoordConv on/off at the
calibrated default.}
...
\paragraph{Training.} Unless a sweep varies it explicitly, every experiment in
this paper uses the same optimizer and schedule: AdamW ($\text{lr}=10^{-3}$,
weight decay $3\times10^{-4}$), batch size $32$, up to $500$ epochs with
early-stopping patience $50$ on the validation loss, over three seeds
($9371$, $9372$, $9373$). The FCNN head is the one defined in
Section~\ref{sec:method}: two hidden layers of width $64$ and $32$ with
dropout $p=0.1$.
...
Before comparing vectorizations or architectures, we quantify the effect
...
(Section~\ref{ssec:exp-performance}, mean $\pm$ sd over 3 seeds).
...
datasets. Coverage $q$ is the one parameter with a genuine, dataset-dependent
...
coverage at $q=0.90$ ($0.195$) and gets markedly worse as $q\to1$; we
therefore keep the shared default $q=0.95$ rather than fit $q$ per dataset.
...
axes) and tensor shapes, refer to REF.
...
\toprule
Vec. & per-channel dim & channels & $\mathcal{L}$ native (T) & $\mathcal{L}$ native (NT) & $\mathcal{L}$ MLP (T) & $\mathcal{L}$ MLP (NT) \\
\midrule
...
\bottomrule
\end{tabular}
\caption{Vectorization methods comparison: ``native'' denote's the methods'
...
so the comparison is calibration-fair, not each-vectorization-optimal;
...
(PI native/T uses the 10-seed default run, all other cells use 3--5 seeds --
see Section~\ref{ssec:encoder-fusion} for the shared training protocol).
Per-channel dim $\times$ channels gives the flattened tensor size actually
fed to the encoder (PI: $24{,}576$; LS: $12{,}288$; SIL: $1{,}536$; BC:
$768$) -- comparisons above are not capacity-matched.}
...
label-shuffle floor (Section~\ref{ssec:exp-summaries}) on both datasets,
...
This sweep was run on the persistence-image arm at the default $q=0.95$
calibration; Section~\ref{ssec:exp-summaries} confirms PI is the arm worth
ablating.

We ran a sweep crossing \emph{encoder mode} (shared-weight vs.\
...
seed noise ($0.122\pm0.007$ vs.\ $0.129\pm0.006$); since tying weights across
scales also cuts the parameter count roughly $K$-fold and removes one design
axis, we adopt the shared encoder going forward. Both convolutional-fusion
variants are unstable, but \emph{only when paired with the shared encoder}:
...
because, together with its shared-encoder counterpart, it is the only
combination that both trains reliably and reaches the best loss; the
convolutional-fusion instability is specifically an interaction with weight
sharing, not a property of convolutional fusion on its own.

The same two concatenation arms (shared and independent encoder) were
...
($\pm0.029$ vs.\ $\pm0.005$), so the shared-encoder choice, if anything,
matters more on the harder five-parameter design distribution -- reinforcing
rather than undercutting the choice made above.
...
% APPENDIX
% #############################################################################
% NOTE -- PURPOSE: the "Appendix A" forward-referenced from
% Section~\ref{ssec:calibration} (twice) and Section~\ref{ssec:exp-summaries}
% -- full calibration detail for every vectorization OTHER than the
% persistence image, one subsection per vectorization. Persistence
% Landscapes, Silhouettes, and Betti/Euler curves are now populated; the
% persistence-statistics floor control (calibration-free -- see
% ssec:exp-summaries -- so there may be nothing to sweep here beyond a
% sanity-check subsection) is still \todo.
% #############################################################################
\appendix

\section{Calibration of Alternative Vectorizations}
\label{app:alt-calibration}
...
(Section~\ref{ssec:exp-performance}, mean $\pm$ sd over 3 seeds). $^\ddagger$
...
report $\mathcal{L}$ (Section~\ref{ssec:exp-performance}, mean $\pm$ sd over
3 seeds).}
