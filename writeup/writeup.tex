...
simplex} on $\pc$, i.e. the collection of all non-empty subsets of $\pc$
    ...
    \begin{definition}[Betti curve]
        For fixed homology dimension $k$, the Betti curve is the right-continuous
        step function counting $k$-features alive at scale $r$:
            \[
...
\todo{Add the tent function $\Lambda_i(t)=\max(0,\min(t-b_i,d_i-t))$ here.
Landscapes, silhouettes and (indirectly) persistence images are all built from
it, so defining it once in Background lets Section 7.1 define three
vectorizations in a paragraph each instead of a page each.}

\todo{Keep ONLY the persistence image definition in full here. Landscape,
silhouette, Betti/Euler and statistics are defined compactly in
Section \ref{ssec:exp-summaries}, where they are evaluated. Do not define them
twice.}

\fig{Schematic: one point cloud $\rightarrow$ its $H_0/H_1$ diagrams $\rightarrow$ the five
vectorizations side by side (PI raster, landscape stack, silhouette curve, Betti
curve, statistics bar). One tight-cluster cloud and one near-Poisson cloud.
This is the figure that makes the paper legible to a reader who does not know
TDA, and it does double duty as the visual key for Section 7.1.}
...
\fig{Canonical identity vs \eqref{eq:varN} as a function of $\nu$, with
...
\todo{PIPELINE EQUATION IS WRONG. The native encoder emits $E=64$, not $128$,
and flat concatenation over $n_k=3$ scales gives $192$, plus the $\log N(x)$
side feature = $193$ into the head. Replace $e \in \R^{128}$ throughout with
$e \in \R^{n_k \cdot E}$ and give the concrete numbers once. Note the MLP
control path uses $E=128$; state that where it is introduced, not here.}

\todo{The $\log N(x)$ side feature is appended in every configuration but is
never mentioned in the paper. It must appear in the pipeline equation and in
Section \ref{sec:problem}: an estimator that is handed the point count is not a
purely topological estimator, and $\lambda = \kappa\mu$ is close to recoverable
from $N$ alone.}
...
\todo{RESOLVE CONTRADICTION WITH CODE. This subsection claims a fallback to a
one-dimensional vectorization when the birth axis is degenerate; the
implementation raises instead. Under DTM the $H_0$ births are nonzero so the
case never fires -- say that, and do not describe a fallback that does not
exist.}
...
\todo{Rewrite declaratively per the agreed pattern: state the shared-weight
encoder + flat concatenation outright, track dimensions end to end
($(N_b,n_k,C,64,64) \to (N_b\!\cdot\!n_k,C{+}2,64,64) \to (N_b,n_k,64) \to
(N_b,193)$), and close with ONE forward-reference sentence to Section
\ref{ssec:exp-ablations}. Delete the posed question and the enumerated menu of
three fusion options -- the alternatives are defined where they are evaluated.}

\todo{Add the CoordConv mechanism claim explicitly: calibration is what makes a
pixel position meaningful across the dataset, and CoordConv is what lets the
network use it. State it as a claim so the ablation below has something to test.}
...
\fix{Naturally, different vectorization techniques require different encoder
architectures to produce the embedding vector. Similarly to
Section REF, we focus on the rich example of the multi-$k$ $DTM_{k}$ persistence image
model and defer to Section REF for an exposition and comparison of the different
vectorizations and encoders used.}
...
\todo{State the greedy selection order in one paragraph: architecture chosen at
a fixed default calibration; calibration then swept at the chosen architecture;
confirmation pass re-running the two leading architectures at the selected
calibration; vectorization-specific parameters swept last with everything else
frozen. A stated order is what makes this defensible -- silence is what invites
the objection.}
...
\todo{Compact definitions, one short paragraph each, all built on the tent
function from Section \ref{ssec:persistence}: landscape (Eq.\ order statistics
of tents), silhouette (persistence-weighted mean of tents, $p$ as an arm),
Betti/Euler (feature counts at each scale, $\chi=\beta_0-\beta_1$), statistics
(18 scalars, floor control). Full calibration and tensor shapes go to Appendix A.}

\todo{FAIRNESS. Silhouette is run as four arms ($p\in\{0,1,2,3\}$) and the
landscape gets a data-derived $K$; persistence images get one fixed
$64\times64$ configuration. Either report SIL as the mean over $p$ with the
breakdown in the appendix, or pre-declare $p=1$ as the headline arm. Do not
compare best-of-four against best-of-one.}
