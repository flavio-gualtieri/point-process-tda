...
\begin{figure}[htbp]
    ...
    \caption{The same $H_1$ diagram -- the most feature-rich diagram among the
    non-outlier bulk (own max persistence at or below the 60th percentile),
    drawn from $4000$ clouds sampled from the production Thomas design
    distribution (Section~\ref{sec:simulator}) -- rendered under the naive
    pooled min/max bound (left) and the per-diagram coverage-quantile bound
    of $q=0.95$, $\alpha=1.05$ (right). Top row: the diagram's own points
    against the calibrated birth-persistence box, with the dashed square
    marking the region zoomed into below; the naive box is stretched by a
    single high-offspring outlier diagram elsewhere in the population, not
    shown. Bottom row: the same physical region of birth-persistence space,
    rendered at each calibration's native pixel density -- visibly coarser
    under the naive bound, since the same $64\times64$ grid is stretched over
    a wider range.}
...
STAT & $18$ (scalars, no raster) & 2 & n/a & n/a & \#\#\# & \#\#\# \\
\bottomrule
\end{tabular}
\caption{Main vectorization comparison. ``native'' is each vectorization's
...
persistence image (the \texttt{fairq095} runs) rather than each
...
$768$) -- comparisons above are not capacity-matched. \#\#\# = not yet run
(STAT and the LS/SIL/BC MLP-path arms).}
...
ablating, so the choice of what to ablate here does not beg the question --
had another vectorization won that comparison, this subsection would describe
a configuration the paper does not ship.
