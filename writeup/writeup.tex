...
Before comparing vectorizations or architectures, we first quantify the effect
...
raster resolution $G$ -- so that the vectorization comparison of
Section~\ref{ssec:exp-summaries} runs at a calibration that is itself
justified rather than assumed. Table~\ref{tab:calibration-sweep} reports the
one-at-a-time sweep, each arm varying a single parameter around the default
...
$\sigma_{\text{pixels}}=0.75$, $G=64$), on the \emph{params} task for both
the Thomas and nested-Thomas design distributions (Section~\ref{sec:simulator}).
The $\sigma_{\text{pixels}}$ arm sweeps $\{0.5,1.0,2.0\}$, which brackets
but does not include the $0.75$ default, so no row in that group is marked
``default''. Diagrams are computed under the DTM filtration fused across
$k \in \{5,10,15\}$ and homology dimensions $H_0{+}H_1$ (six PI channels
total, $G \times G$ each), fed to a shared-weight CNN encoder (embedding
dim $64$, conv channels $32$, $64$, $128$, average pooling, dropout $0.2$)
with a two-layer MLP head (hidden dims $64$, $32$, dropout $0.1$),
concatenation fusion across $k$, trained with AdamW ($\text{lr}=10^{-3}$,
weight decay $3\times10^{-4}$, batch size $32$, up to $500$ epochs with
early-stopping patience $50$). Cells report $\mathcal{L}$
(Section~\ref{ssec:exp-performance}, mean $\pm$ sd over 3 seeds: 9371, 9372,
9373). $^\dagger$ marks configurations that diverge on at least one of the
...
diverged seed(s) and should not be read as a stable estimate. Excluding the
$^\dagger$ rows, resolution $G=128$ is the only arm that is unambiguously
bad -- $G\in\{32,64\}$, $\alpha$, and $\sigma_{\text{pixels}}$ all vary
within one seed-standard-deviation of the default across both datasets.
Coverage $q$ is the one parameter with a genuine, dataset-dependent
...
Section~\ref{ssec:calibration} calibrates the persistence image's birth/persistence
...
CNN encoder with weights tied across the DTM scales $k \in \{5,10,15\}$ (folded
into the batch dimension for a single forward pass), concatenation fusion, and a
...
design distributions (Section~\ref{sec:simulator}). Diagrams are computed under
the DTM filtration fused across $k \in \{5,10,15\}$ and homology dimensions
$H_0{+}H_1$ (two landscape channels per scale, six total), fed to the same
CoordConv-augmented CNN encoder used for the persistence-image sweep of
Table~\ref{tab:calibration-sweep} (shared weights across $k$, embedding dim
$64$, conv channels $32$, $64$, $128$, average pooling, dropout $0.2$) with a
two-layer MLP head (hidden dims $64$, $32$, dropout $0.1$), concatenation
fusion across $k$, trained with AdamW ($\text{lr}=10^{-3}$, weight decay
$3\times10^{-4}$, batch size $32$, up to $500$ epochs with early-stopping
patience $50$). The diagram-grid coverage $q = 0.95$ (Section~\ref{ssec:calibration})
is fixed across every arm; $K$-selection uses coverage $c=0.99$ and cap
$K_{\max}=16$, both fixed except in the third group. Cells report $\mathcal{L}$
(Section~\ref{ssec:exp-performance}, mean $\pm$ sd over 3 seeds: 9371, 9372,
9373). $^\ddagger$ the $G=512$, $K=8$ arm has a $4096$-d feature per $(h,k)$,
matching the $64\times64$ persistence-image budget exactly; the other three
rows of that group hold $G=128$ fixed and vary only $K$, so this row is not
raster-size-matched to them. \textbf{The coverage rule resolved to $K=16$ --
the cap -- on every arm and seed underlying the first two groups}, so the $G$
and $\tau$ sweeps report robustness at a $K$ already pinned by $K_{\max}$, not
by $\tau$ itself; their near-flat $\mathcal{L}$ (differences mostly within one
seed-sd) is consistent with sweeping a quantity ($G$, $\tau$) that is not
presently the binding constraint on $K$. The fixed-$K$ group is the
informative one for this rule: the ``data-derived'' row lands within seed
noise of the explicit $K=16$ row on both datasets, exactly as expected since
both resolve to the identical ($G=128$, $K=16$) configuration, while $K=4$ and
$K=8$ are both markedly and monotonically worse. Within the range tested, more
landscape layers help and $K_{\max}$ -- not $\tau$ -- is the binding
constraint; distinguishing the rule's own threshold from its cap would require
re-running with a larger $K_{\max}$.}
