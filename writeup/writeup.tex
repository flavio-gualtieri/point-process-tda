...
% persistence image, one subsection per vectorization. Only Persistence
% Landscapes is populated so far; Silhouettes, Betti/Euler curves, and the
% persistence-statistics floor control are still \todo.
% #############################################################################
\appendix
...
\todo{silhouette calibration sweep: grid resolution $G$ and the amplitude
exponent $p \in \{0,1,2,3\}$ (Section~\ref{ssec:exp-summaries}), same table
format as Table~\ref{tab:calibration-sweep-landscape}.}

\subsection{Betti and Euler Curves}
\label{app:calib-betti}
\todo{Betti/Euler curve calibration sweep: grid resolution $G$ only (no
$K$-analogue, since Betti curves have no layer-count axis).}
