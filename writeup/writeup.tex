...
We implemented and tested \todo{N} vectorizations of the persistence diagram,
whose relative efficacies are the subject of Section \ref{ssec:exp-summaries}.
For the remainder of this section we describe the pipeline as instantiated with
persistence images computed under the $DTM_k$ filtration, which is the
configuration carried through the rest of the paper. We give this configuration
in full because it is the one that raises design questions the alternatives do
...
filtration of Example \ref{ex:dtm} rather than the canonical Rips filtration:
...
\ref{ssec:exp-summaries} reports the comparison that bears this out.
Furthermore, the parameter $k$ allows us to compute summaries at various scales,
which may be combined to provide the model with a more complete picture.
