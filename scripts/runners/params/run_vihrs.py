# scripts/runners/params/run_vihrs.py
...
Purpose
-------
This repo's own pipeline (scripts/runners/params/run_params.py) trains one
network per TDA-derived feature (raw point cloud, persistence images, Betti
curves, pairwise/correlation features). This script trains an INDEPENDENT
network using the classical-statistics feature set from the paper above, on
the SAME data (data/params/2d/thomas/clouds.pkl + adversarial_clouds.pkl), the
SAME 10 canonical seeds and 70/15/15 split convention as run_params.py, and
writes results in the SAME results.pt / results.json / model.pt schema, under

    results/params/2d/thomas/vihrs/seed_<seed>/

so it drops straight into models/evaluate_seeds.py for a side-by-side
comparison:

    python models/evaluate_seeds.py --models-root results/params/2d/thomas \\
        --features betti_0 betti_1 pairwise pi_0 pi_1 raw_pc vihrs

This file does NOT import or modify anything under src/cloudforger/nn — it is
a fully independent implementation of the paper's own recipe. It DOES reuse
two small, framework-agnostic pieces of this repo, on purpose, for a fair and
exact comparison: models/evaluate.py's marginal-metric formulas (so "same
metrics" means literally the same code, not a re-derivation that could drift),
and models/mincontrast.py's minimum-contrast estimator (the paper's own
evaluation protocol calls for a comparison against minimum contrast, and this
repo already has that estimator).
...
* Ripley's isotropic edge correction has no existing implementation anywhere
  in this repo (models/mincontrast.py only has a translation/minus-sampling
  correction) and neither spatstat/rpy2 nor astropy/pointpats is installed,
  so it is implemented directly below (`_isotropic_l_minus_r`) from the
  standard closed-form rectangle formula: for a point at distances
...
* r-grid: spatstat's Kest/Lest default is rmax = min(side/4, sqrt(1000 / (pi
  * lambda))), evaluated with 513 equally-spaced points. The second
  ...
  dataset). This also matches the r_high=0.25 convention already used for
  this exact dataset in models/evaluate.py and models/mincontrast.py.
...
* Target and n(x) standardization: the paper's step 2(d) is explicit here
  ...
  data/params/2d/thomas/cloud_generation_manifest.yaml's "random" design
  block (the one actually used for this dataset — its n_param_vectors: 5000
  matches this dataset's 4250 train_test + 750 adversarial rows exactly)
  samples parent_intensity/mean_offspring/cluster_scale with scale: log,
  confirmed empirically (each parameter has ~50-60% of its mass below its
  *geometric* midpoint vs. ~80-87% below its *linear* midpoint — the
  log-uniform signature). The paper's own LGCP/Strauss/LGCP-Strauss examples
  sample parameters on modest linear-uniform ranges, where plain z-score is
  the right call; for this dataset's wide log-spanning ranges
  (parent_intensity 10-200, cluster_scale 0.005-0.1) plain z-score would
  leave MSE dominated by the top of the range, so log+z-score is kept. Same
  reasoning for n(x): counts span a similarly wide, right-skewed range.
* Paper defaults used as-is, including the training procedure: batch_size=100,
  epochs=20 (Appendix A.2 — chosen once via a held-out-curve check that
  showed "no apparent problem with overfitting" at that count; the paper
  then just trains a fresh network for exactly that many epochs and
  evaluates the final network — no early stopping/best-checkpoint
  selection), Adam with lr=0.001 and no weight decay, no dropout (Figure 1's
  architecture has neither). This repo's other methods use
  best-val-checkpointing + weight_decay=1e-4 + 500 epochs for their own
  overfitting reasons (see cloudforger.nn.experiments.base); that convention
  does not carry over here since the point of this module is a literal,
  standalone replication of Vihrs (2022)'s own recipe, not training-budget
  parity with the TDA methods it gets benchmarked against elsewhere in this
  repo.

Nothing here is cached or shared with run_params.py's data pipeline; the
L(r)-r + n(x) feature extraction (the expensive, O(points^2) part) is cached
to a sibling .npz file next to each source pickle so repeated seeds/re-runs
don't recompute it.
"""
...
import evaluate as classical_evaluate  # models/evaluate.py — reused for identical marginal-metric formulas

try:
    import mincontrast as mc  # models/mincontrast.py — optional baseline (paper's own evaluation protocol)

    _HAVE_MINCONTRAST = True
except ImportError:
    mc = None  # type: ignore[assignment]
    _HAVE_MINCONTRAST = False
    warnings.warn("models/mincontrast.py not importable — minimum-contrast comparison plots will be skipped.")
...
# Must match models/evaluate_seeds.py::DEFAULT_SEEDS (duplicated here, not
# imported, to keep this script independent of the rest of the NN pipeline).
SEEDS: list[int] = [
    56245,
    189089,
    2342344,
...
# Matches cloudforger.nn's label_names order for this process (see
# N_PARAMS = {"thomas": 3, ...} in src/cloudforger/nn/experiments/base.py).
LABEL_NAMES: tuple[str, ...] = ("parent_intensity", "mean_offspring", "cluster_scale")

DEFAULT_CLOUDS = PROJECT_ROOT / "data" / "params" / "2d" / "thomas" / "clouds.pkl"
DEFAULT_ADVERSARIAL = PROJECT_ROOT / "data" / "params" / "2d" / "thomas" / "adversarial_clouds.pkl"
DEFAULT_OUTPUT_ROOT = PROJECT_ROOT / "results" / "params" / "2d" / "thomas" / "vihrs"
...
MC_SEED = 420937  # matches models/evaluate.py's default mc_seed, for reproducibility of the bonus comparison
...
def extract_features(records: list[dict[str, Any]], r_grid: np.ndarray, *, tag: str = "") -> dict[str, np.ndarray]:
    n = len(records)
    m = len(r_grid)
...
def get_features(
    ...
    *,
    ...
    features = extract_features(records, r_grid, tag=tag)
...
def fit_log_zscore(values: np.ndarray) -> dict[str, np.ndarray]:
    """Mirrors _normalize_labels_by_name in src/cloudforger/nn/experiments/base.py:
...
def train_val_test_indices(
    ...
    """Identical index partition to cloudforger.nn.splits.train_val_test_split
    for the same n and seed (torch.randperm sliced at the same cumulative
    offsets), so every method trains/tests on exactly the same clouds."""
...
def run_one_seed(
    ...
    *,
    ...
    # Fit every normalization statistic on the train split ONLY (paper step
    # 2d: "the training data" / item 3b freezes those stats for test), then
    # apply the frozen stats to val/test/adversarial below. Previously these
    # were fit on the whole (train+val+test) population before splitting --
    # a train/test leakage bug relative to the paper's own protocol.
    ...
    # val_loss is tracked purely as a diagnostic curve (and for parity with
    # this repo's other methods' history/plots) -- it is NOT used to select
    # a checkpoint. The paper trains a fresh network for a fixed n_epochs
    # (chosen once, up front, via exactly this kind of curve -- see Appendix
    # A.2 / Figure 19) and evaluates whatever network exists at the end.
    ...
    # ---- save, matching the existing results.pt / results.json / model.pt schema ----
    cfg = {
        "task": "params",
        "process": "thomas",
        "method": "vihrs",
    ...
    # ---- same marginal metrics as models/evaluate.py, on de-standardized test predictions ----
...
def prepare_data(
    ...
    *,
    ...
    L(r)-r + n(x) feature set that run_one_seed needs. Called once and reused
    across every seed by both main() below and dtm_experiment/train.py."""
    ...
    train_features = get_features(
        train_records, r_grid, cache_path=train_cache, force=force, tag="train_test"
    )
    ...
    if adversarial_path is not None and adversarial_path.exists():
        ...
        adversarial_features = get_features(
            adv_records, r_grid, cache_path=adv_cache, force=force, tag="adversarial"
        )
    else:
...
def main(argv: list[str] | None = None) -> dict[int, dict[str, Any]]:
    ...
    print(
        "Compare against the existing methods with:\n"
        f"  python models/evaluate_seeds.py --models-root {PROJECT_ROOT / 'results' / 'params' / '2d' / 'thomas'} "
        "--features betti_0 betti_1 pairwise pi_0 pi_1 raw_pc vihrs"
    )
    return summary
