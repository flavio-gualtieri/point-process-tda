# scripts/runners/params/run_new_feature.py
...
writes results in the SAME results.pt / results.json / model.pt schema, under
    ...
    results/params/2d/thomas/new_feature/seed_<seed>/
...
comparison:
    ...
    python models/evaluate_seeds.py --models-root results/params/2d/thomas \\
        --features betti_0 betti_1 pairwise pi_0 pi_1 raw_pc new_feature
...
* Target AND n(x) standardization: the paper trains with plain MSE but does
  not say whether inputs/targets are standardized first. We log-transform +
  z-score the 3 targets exactly like
  cloudforger.nn.experiments.base._normalize_labels_by_name (fit on the whole
  clouds.pkl, before the split), and similarly log+z-score n(x) before
  concatenating it in the merge layer. This is a necessary step regardless
  (the 3 raw parameters live on very different scales: parent_intensity
  10-200, cluster_scale 0.005-0.1), and as a side benefit means the reported
  loss is already directly on the same scale as every other method's
  results.pt/history — no separate "conversion" step is needed to overlay
  training curves; see `metrics.json` / the plots/ folder for the
  de-standardized, real-unit comparison instead.
* Paper defaults used as-is: batch_size=100, epochs=20, Adam default
  settings (lr=0.001, which is also this repo's existing default for every
  other method).
...
DEFAULT_OUTPUT_ROOT = PROJECT_ROOT / "results" / "params" / "2d" / "thomas" / "new_feature"
...
def _mincontrast_comparison(
    ...
    *,
    ...
    for j, name in enumerate(label_names):
        ...
        ax.scatter(t, nn_pred_raw[:, j], s=10, alpha=0.4, color="#2a78d6", label="new_feature (NN)")
        ax.set_xlabel(f"true {name}")
        ax.set_ylabel(f"estimated {name}")
...
def run_one_seed(
    ...
    *,
    ...
    label_norm = fit_log_zscore(train_features["targets"])
    n_norm = fit_log_zscore(train_features["n_points"])
    ...
    seq = train_features["l_minus_r"].astype(np.float32)
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        print(f"[new_feature seed={seed}] epoch {epoch:3d} | train {train_loss:.4f} | val {val_loss:.4f}")

    model.load_state_dict(best_state)
    test_loss = evaluate_loss(model, test_loader, loss_fn, device)
    print(f"\n[new_feature seed={seed}] test loss {test_loss:.4f}")
    ...
    if adversarial_features is not None:
        ...
        adv_seq = adversarial_features["l_minus_r"].astype(np.float32)
        ...
        print(f"[new_feature seed={seed}] adversarial loss {adversarial_loss:.4f}")
    ...
    if not skip_mincontrast:
        try:
            ...
            warnings.warn(f"[new_feature seed={seed}] mincontrast comparison failed: {exc!r}")
