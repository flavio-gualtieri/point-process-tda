...
def _run_raw_pc(cfg: dict):
    ...
    if cfg["process"] == "matern":
        head = ParameterEstimator(embedding_dim=cfg["embedding_dim"], n_params=2)
    if cfg["process"] == "thomas":
        head = ParameterEstimator(embedding_dim=cfg["embedding_dim"], n_params=3)
...
def _run_pi(cfg: dict):
    ...
    for hom_dim in (0, 1):
        ...
        results[hom_dim] = {"history": history, "test_acc": test_acc}
...
def _run_pairwise(cfg: dict):
    ...
    dataset = CorrelationFeatureDataset(features, labels)
    loaders = _make_loaders(dataset, cfg)

    encoder = StatsEncoder(input_dim=dataset.input_dim, embedding_dim=cfg["embedding_dim"], hidden_dims=cfg["hidden_dims"])
    if cfg["process"] == "matern":
        head = ParameterEstimator(embedding_dim=cfg["embedding_dim"], n_params=2)
    if cfg["process"] == "thomas":
        head = ParameterEstimator(embedding_dim=cfg["embedding_dim"], n_params=3)
