...
#SBATCH --gres=gpu:1
#SBATCH --array=0-49
#SBATCH --output=logs/fusion_thomas_%A_%a.out
#SBATCH --error=logs/fusion_thomas_%A_%a.err

# PH + L(r) FUSION SWEEP -- THOMAS, plain DTM k=5.
#
...
#
# Submit from the repo root:  sbatch slurm/fusion_thomas.sh
...
CFG=configs/runs/thomas/fusion_k5.yaml
...
t="${SLURM_ARRAY_TASK_ID:?run this as an array job (sbatch slurm/fusion_thomas.sh)}"
...
req = ["data/thomas/dtm_k5/diagrams.pkl", "data/thomas/dtm_k5/adversarial_diagrams.pkl",
       "data/thomas/clouds.pkl", "data/thomas/adversarial_clouds.pkl"]
npz = ["data/thomas/clouds.lfunc_cache.npz", "data/thomas/adversarial_clouds.lfunc_cache.npz"]
bad = []
for f in req:
    try:
