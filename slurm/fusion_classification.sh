...
#SBATCH -J fusion_classification
#SBATCH -p sae
...
#SBATCH --gres=gpu:1
#SBATCH --array=0-49
#SBATCH --output=logs/fusion_classification_%A_%a.out
#SBATCH --error=logs/fusion_classification_%A_%a.err

# PH + L(r) FUSION SWEEP -- CLASSIFICATION, plain DTM k=5.
#
...
#
# Submit from the repo root:  sbatch slurm/fusion_classification.sh
...
t="${SLURM_ARRAY_TASK_ID:?run this as an array job (sbatch slurm/fusion_classification.sh)}"
...
req = ["data/classification/dtm_k5/diagrams.pkl", "data/classification/dtm_k5/adversarial_diagrams.pkl",
       "data/classification/clouds.pkl", "data/classification/adversarial_clouds.pkl"]
npz = ["data/classification/clouds.lfunc_classify_cache.npz", "data/classification/adversarial_clouds.lfunc_classify_cache.npz"]
bad = []
for f in req:
    try:
