...
#SBATCH -J pi_multik_calib_sigma_nested_thomas
#SBATCH -p sae
...
#SBATCH --gres=gpu:1
#SBATCH --array=0-8
#SBATCH --output=logs/pi_multik_calib_sigma_nested_thomas_%A_%a.out
#SBATCH --error=logs/pi_multik_calib_sigma_nested_thomas_%A_%a.err

# PI Gaussian kernel width (sigma_pixels) sweep, nested_thomas, FUSED DTM
# k=5,10,15 (one 3-channel multi-scale model per grid point -- shared-weight
# encoder + flat concat fusion, the pi_multik defaults -- NOT three
# independent single-k models), 3 values of Gaussian-kernel width in pixels
...
# results/nested_thomas/dtm_k5+10+15/pi_multik/_runs/calib_sigma<value>/seed_<seed>/
...
#   sbatch slurm/pi_multik_calib_sigma_nested_thomas.sh
