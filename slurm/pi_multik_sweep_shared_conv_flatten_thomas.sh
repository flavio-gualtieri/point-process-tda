...
#SBATCH -J pi_multik_sweep_shared_conv_flatten_thomas
#SBATCH -p sae
...
#SBATCH --gres=gpu:1
#SBATCH --array=0-4
#SBATCH --output=logs/pi_multik_sweep_shared_conv_flatten_thomas_%A_%a.out
#SBATCH --error=logs/pi_multik_sweep_shared_conv_flatten_thomas_%A_%a.err

# pi_multik encoder/fusion sweep, thomas -- combo 3/6: encoder_mode=shared,
# fusion_mode=conv, fusion_pool=flatten (shared-weight encoder, Conv1d over
# the k axis, position-preserving flatten instead of avg-pool -- see
# ConvFusion's docstring in src/cloudforger/encoders/scaleconv_pi.py).
# thomas counterpart of pi_multik_sweep_shared_conv_flatten.sh
# (nested_thomas). FUSED DTM k=5,10,15
