...
#SBATCH --gres=gpu:1
#SBATCH --array=0-49
#SBATCH --output=logs/dimsplit_thomas_%A_%a.out
#SBATCH --error=logs/dimsplit_thomas_%A_%a.err

# CHECK A -- per-dimension H0/H1 encoder ablation.  THOMAS parameter
# estimation (parent_intensity, mean_offspring, cluster_scale), DTM k=5 only.
# This is the NEGATIVE CONTROL: H1-only was ~chance on Thomas in the paper's
# branch table, so a per-dim H1 encoder is not expected to help here.
#
#   base config   configs/runs/thomas/dimsplit_k5.yaml
...
#
#   Core arms only (0-2) first pass:  sbatch --array=0-29 slurm/dimsplit_thomas.sh
#   Add the capacity controls only if the core arms show a signal.
#
# Results (resumable):
#   results/thomas/dtm_k5/pi_multik/_runs/<run-tag>/seed_<seed>/
#   results/thomas/dtm_k5/pi_multik_dimsplit/_runs/<run-tag>/seed_<seed>/
# Compare arms against dimsplit__shared_e64 here (NOT the fused-k paper
# table), paired seed->seed.
#
# Rerun specific (arm,seed) failures by task id:
#   sbatch --array=<comma,ids> slurm/dimsplit_thomas.sh
#
# Submit from the point-process-tda repo root:
#   sbatch slurm/dimsplit_thomas.sh
...
CFG=configs/runs/thomas/dimsplit_k5.yaml
...
t="${SLURM_ARRAY_TASK_ID:?run this as an array job (sbatch slurm/dimsplit_thomas.sh)}"
...
python - <<'PYEOF'
import pickle, sys
req = ["data/thomas/dtm_k5/diagrams.pkl",
       "data/thomas/dtm_k5/adversarial_diagrams.pkl",
       "data/thomas/clouds.pkl",
       "data/thomas/adversarial_clouds.pkl"]
bad = []
for f in req:
    try:
