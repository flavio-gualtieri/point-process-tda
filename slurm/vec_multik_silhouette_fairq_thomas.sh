...
#SBATCH -J vec_multik_silhouette_fairq_thomas
#SBATCH -p sae
...
#SBATCH --gres=gpu:1
#SBATCH --array=0-5
#SBATCH --output=logs/vec_multik_silhouette_fairq_thomas_%A_%a.out
#SBATCH --error=logs/vec_multik_silhouette_fairq_thomas_%A_%a.err

# Inherited-q fairness check, SIL-1 arm (silhouette, p=1.0, native
# encoder), thomas, FUSED DTM k=5,10,15. Sibling scripts:
# vec_multik_landscape_fairq_thomas.sh (LS arm), betti_multik_fairq_thomas.sh
# (BC arm) -- see the LS script's header for the full fairness-check
# rationale (3 vectorizations x 2 q values = 6 configuration points, each
# across these same 3 seeds).
#
...
# configs/runs/thomas/vec_multik_silhouette_native.yaml (G=128, p=1.0,
# homology_dims=[0,1] already set there, untouched).
#
# Each value gets its own --run-tag, so results land under
# results/thomas/dtm_k5+10+15/vec_multik_silhouette_native/_runs/fairq<value>/seed_<seed>/
