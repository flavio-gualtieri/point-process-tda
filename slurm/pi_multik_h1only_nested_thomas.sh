...
#SBATCH -J pi_multik_h1only_nested_thomas
#SBATCH -p sae
...
#SBATCH --gres=gpu:1
#SBATCH --array=0-4
#SBATCH --output=logs/pi_multik_h1only_nested_thomas_%A_%a.out
#SBATCH --error=logs/pi_multik_h1only_nested_thomas_%A_%a.err

# [PI | H1 | DTM{5,10,15} | 5]: nested_thomas counterpart of
# pi_multik_h1only_thomas.sh -- see that file's header for the full
# rationale. configs/runs/nested_thomas/pi_multik.yaml, AT THE NEWLY
# ESTABLISHED CALIBRATION already baked into that config.
#
# --run-tag h1only ->
# results/nested_thomas/dtm_k5+10+15/pi_multik/_runs/h1only/seed_<seed>/.
#
