...
#SBATCH -J pi_multik_sweep_independent_conv_avg_aux
#SBATCH -p sae
...
#SBATCH --gres=gpu:1
#SBATCH --array=0-4
#SBATCH --output=logs/pi_multik_sweep_independent_conv_avg_aux_%A_%a.out
#SBATCH --error=logs/pi_multik_sweep_independent_conv_avg_aux_%A_%a.err

# pi_multik encoder/fusion sweep, WITH auxiliary variables -- combo 5/6:
# encoder_mode=independent, fusion_mode=conv, fusion_pool=avg,
# include_entropy=true. See pi_multik_sweep_shared_concat_aux.sh's header
# for what include_entropy adds. Same architecture as
# encsweep_independent_conv_avg (showed seed-dependent training collapse in
# the original sweep -- see the loss-comparison report). nested_thomas,
...
python -u scripts/train.py configs/runs/nested_thomas/pi_multik.yaml \
  --seed "$SEED" \
  --run-tag encsweep_independent_conv_avg_aux \
