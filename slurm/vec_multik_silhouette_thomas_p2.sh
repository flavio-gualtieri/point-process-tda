...
#SBATCH -J vec_multik_silhouette_thomas_p2
#SBATCH -p sae
...
#SBATCH --gres=gpu:1
#SBATCH --array=0-9
#SBATCH --output=logs/vec_multik_silhouette_thomas_p2_%A_%a.out
#SBATCH --error=logs/vec_multik_silhouette_thomas_p2_%A_%a.err

# p-sweep sibling of vec_multik_silhouette_thomas.sh (p=1, already run):
# same vectorization=silhouette, encoder_path=native config
# (configs/runs/thomas/vec_multik_silhouette_native.yaml), p overridden to
# 2 via --set method.params.p=2 -- p is an EXPERIMENTAL AXIS (run every
...
# under vec_multik_silhouette_native_p2/, separate from p=1's
...
#   sbatch slurm/vec_multik_silhouette_thomas_p2.sh
