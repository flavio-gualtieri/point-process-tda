...
#SBATCH -J pi_multik_sweep_independent_conv_flatten
#SBATCH -p sae
...
#SBATCH --gres=gpu:1
#SBATCH --array=0-4
#SBATCH --output=logs/pi_multik_sweep_independent_conv_flatten_%A_%a.out
#SBATCH --error=logs/pi_multik_sweep_independent_conv_flatten_%A_%a.err

# pi_multik encoder/fusion sweep -- combo 6/6: encoder_mode=independent,
# fusion_mode=conv, fusion_pool=flatten (== the old pi_multik_towers
# preset: independent per-k encoders, Conv1d over k, k positions flattened
# rather than pooled -- see EncoderBank's and ConvFusion's docstrings in
# src/cloudforger/encoders/{encoder_bank,scaleconv_pi}.py). nested_thomas,
